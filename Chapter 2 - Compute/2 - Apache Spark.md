...
trancate: bool or int - עם יש ערך מספרי, חותך מחרוזות עם יותר תווים מהמספר.
...
לפי מה שראיתי, אם משתמשים בlocal mode, אז רואים לוגים של spark.
לא הצלחתי לבדוק בcluster mode אבל לפי הדוקומנטציה, הclient עושה exit לאחר הsubmit ולכן אני מניח שלא נראה כלום.
כלומר נראה outputs ואת ההדפסות/לוגים, איפה שהstdout של הדרייבר.
...
דבר ראשון הtasks יוקצו לexecutors אחרים ואם אנחנו בdynamic allocation וצריך משאבים, אז יוקם עוד executor אבל ברמה הבסיסית לא.
...
היחידת מידה היא core כלומר כמות ליבות

19. כמה executors יופיעו בspark UI עבור כמות מבוקשת כלשהי ?

יופיעו כמות הexecutors הקיימים והדרייבר ולא בהכרח הכמות המבוקשת.
...
ניתן להשתמש בspark history server.
כלומר צריך להרים את התהליך הזה ולאפשר לו לקרוא את הלוגים מאיפשהו.

5. לגבי אותו רכיב מהשאלה הקודמת, באיזה אחסון הוא שומר את המידע והאם הוא instance לכל תהליך או עובד בצורה שונה ?

מרימים אחד ודרכו ניתן לראות תהליכים שונים.
הוא שומר את האחסון כלומר איפה שהלוגים נשמרים זה קונפיגורבילי.
הוא פועל לפי תיקייה ספציפית או נתיב ספציפי.
