...
ניתן לטפל ב referral loops על ידי זה שהלקוח יזכור איזה בקשות שלח לאילו שרתים ויצא במידה ושלח אותה בקשה פעמים, או על ידי מונה שיגביל את כמות ה referrals שניתן לעבור אליהן.

כשמקבלים שגיאת insufficient access זה אומר שאין לנו הרשאות לבצע פעולה מסויימת וצריך להתחבר למשתמש עם הרשאות חזקות יותר.

## Q&A

1. האם יכול להיות יותר ממאסטר אחד באנסמבל ?



2. איזה מידע יכול להשמר בפועל ב znode ?

המידע שניתן לכתוב לznode הוא מערך של בייטים.

3. איך נראה znode ? האם הוא קריא ?

המידע נשמר ב memory ולכן אני לא בטוח מה לענות על השאלה הזאת, במיוחד כשהמידע שמכניסים הוא מערך של בייטים. אין לו פורמט קריא.

4. מה ההגדרה של קובץ ?

קובץ הוא אובייקט שניתן לקרוא ממנו לכתוב אליו או שניהם. ויש לו מטא דאטא ששמור ב inode.
http://pubs.opengroup.org/onlinepubs/9699919799/basedefs/V1_chap03.html#tag_03_164

5. למה צריך ZK ? למה לא KV ?

זה נכון שאפשר תיאורטית לבצע ב DBפעולות כמו מנעולים ו leader election, אבל פרטית, זה יכול להגיע למצבים שיש הרבה אוברהד של טרנזקציות ולוקים ויכולים לקרות הרבה דדלוקים ZK נותן את כל היכולות האלה כולל ephemeral nodes ו watch, בחבילה אחת בצורה שממומשת יותר טוב ויותר בנויה ל usecases האלה.
זה כמו לשאול למה לנסוע ברכב אם אפשר תיאורטית לשים גלגיליות ולהתגלגל לכל מקום.

6. למה צריך leader בשביל כתיבות ?

כדי שהzxid יוכל באמת לעבוד כמו שצריך בסדר עולה.

8. מה המידע שעובר ב heartbeat ? כל כמה זמן ?

heartbeat קורה כל כמות זמן שמוגדר בקונפיגורציה תחת השדה tickTime.
הבקשה שקורית בפועל היא פינגים לשרת.

9. מה זה discovery service ?

זה המכניזם שדרכו תהליכים שונים יכולים למצוא את הכתובות אחד של השני ולתקשר ביניהם.
אפשר לממש את זה בזוקיפר אם כל service ירשום את עצמו - הכתובת שלו ב ephemeral sequential znode בנתיב ידוע. ולהגדיר עליו watches במידה ויש שינויים.

10. האם יש TTL על znode ?

כן, ניתן להגדיר TTL במילי שניות, ואז אם ה TTL חלף ואין לו בנים אז הוא מיועד להמחק על ידי השרת. צריך לאפשר שימוש ב TTL nodes בקונפיגורציה zookeeper.extendedTypesEnabled.

11. האם יכול להיות znode בתוך sequential או ephemeral ?

בתוך ephemeral לא, בתוך sequential כן.

12. איך ZK יודע כמה שרתים אמורים להיות ב cluster ?
    איך כל אחד יודע מה ה id שלו ?

רשום בקובץ קונפיגורציה את הכתובות של כל השרתים ב cluster וה id שלהם, שרושמים אותו גם בתיקיית dataDir בקובץ שנקרא myid שיושב על השרת עצמו. הנתיב של הdataDir, נקבע גם כן בקונפיגורציה.

13. מה זה transaction log ?

מכיוון שהמידע בפועל נשמר בזיכרון, כל פעולת כתיבה בזוקיפר נכתבת ב transaction log ונשמר על הדיסק.
פעולת כתיבה נחשבת מוצלחת רק אם זה נרשם גם לtransaction log.

14. מה זה key tab ?

זה קובץ קריפטוגרפי שמכיל שמות של principals  והמפתחות שלהם.
ואז ניתן להתאמת מול השרת באמצעות הקובץ הזה.

15. מה התוקף של ticket והאם זה קונפיגורבילי ?

יש שני סוגים:
ticket lifetime - הזמן שיש לכרטיס הנוכחי
renewable lifetime - הזמן שאפשר לחדש כרטיס לפני שצריך להוציא אחד חדש לגמרי.

בדרך כלל התוקף הוא ליום עבודה, למשל 8 שעות או 10 שעות אם אפשרות לחדש למשך כמה ימים.

16. האם שלב האימות וקבלת TGT יכול להתבצע עם LDAP ?

כן, וזה אפילו מקובל, בהנחה והבנתי את השאלה נכון כלומר האם המידע בשרת kerberos יכול להשמר ב ldap.
...
- _Hadoop Security_ chapter in any modern Hadoop book for integration examples.
