...
הnamespace management זה בעצם איחוד לוגי של כמה טבלאות תחת אובייקט אחד שהוא הnamespace.
...
(משהו רנדומלי כדי שירשם נורמלי) table formats הם פורמטים לאיגוד מידע גולמי תחת טבלה אחידה עם ניהול metadata, schema, ACID.
...
כלומר המידע בקטלוג אומר לנו באיזה קבצים לחפש ואילו הmrtadata בקובץ, מאפשר לנו לקרוא אותו ולחפש בו מידע ספציפי יותר ביעילות.
...
6. **Interoperability & Ecosystem:** Describe how catalogs and formats enable
   ...
   כיוון שמנגישים רק מטא דאטא ולא את המידע עצמו, מנועים כמו spark, trino יכולים לתמוך בתשאול כלפי הקטלוג מבלי להכיר בכלל איך המידע שמור בפועל (לא בהכרח כל המידע באותו מקום) ולכן כל מנוע כזה יכול לתשאל את הקטלוג ולחפש את המידע בהתאם.
   מה שמאפשר את זה הוא בדרך כלל האבסטרקציה לסוג הקובץ.

   חשוב שיהיה standart אחיד למידע כדי שנוכל לתשאל אותו יותר בקלות ולאפשר אינטגרציה בין פיסות מידע שונות ברמה גבוהה יותר ובקלות.
   הspec הוא בעצם מגדיר את הסטנדרט ובעצם את המושגים ואיך הפורמט הטבלאי נראה בפועל.

## Q&A

1. שוב, למה צריך service ? מגבלות של derby

כשמתמשים בembedded db יש בעיות עם הרבה כותבים ובאופן כללי עם concurrency ומקביליות, כי הDB coupled לאפליקציה הספציפית שהוא רץ בתוכה.
בעצם הservice מנהל את הגישות לDB כדי לאפשר את כל הדברים האלה. ובפרט לאפשר גישה לDB מעוד אפליקציות.

2. למה בהייב יש ACID רק על ORC ?

התשובה הפשוטה היא שזה מומש רק על ORC
אבל ORC יותר אופטימלי כי חיפושים בו לפי Row id יותר יעילים בגלל האינדקס.

3. למה קשה לממש ACID בהייב ?

המבנה ההיררכי מקשה על isolation מה שמקשה מאוד בניהול מנעולים, וקבצים בדרך כלל הם column oriented מה שמקשה על חיפושים וACID לפי שורה.
